...
def embedded_QRS_circ_reco(params, user_params):
...
class embedded_QRS_model2():
    def __init__(self, R, train_steps, train_sets, layers):
        self.R = R
        ...
        for t_set in range(self.train_sets):
            ...
            for user in range(defines._NUM_OF_USERS):
                ...
                for t_step in range(10):
                    params, user_params = opt_item_item.step(lambda x, y: self.total_cost_embedded_QRS(user, x, y),
                                                             params, user_params)
            ...
            for t_step in range(self.train_steps):
                for user in range(defines._NUM_OF_USERS):
                    user_params = self.user_params[user]
    ...
    # ----------------------------------------------------- COST -------------------------------------------------------
    def total_cost_embedded_QRS(self, user, params, user_params):
        total_cost = 0

        # running the circuit
        probs = embedded_QRS_circ_reco(params, user_params)
    ...
    def get_recommendation(self, user, uninteracted_items, removed_movie):
        ...
        user_params = self.user_params[user]
        probs = embedded_QRS_circ_reco(self.params, user_params)
        # DEBUG
